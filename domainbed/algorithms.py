    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for domain_idx in range(self.current_domain):
            if domain_idx in self.buffer:
                ...
                constraint_slacks.append(slack.detach().clone())
        ...
        # Primal update (Line 6-7)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()

        # Dual update (if there are previous domains) (Line 8-9)
        ...
        # If we're moving to a new domain (part of outer loop in Algorithm 1)
        if self.update_count % self.hparams.get('domain_steps', 1000) == 0:
            ...
            if new_domain < len(minibatches):
                ...
                # Partition buffer based on dual variables (Line 10)

...
class SeqCNN(nn.Module):
    ...
    def features(self, x):
        ...
        h = torch.cat(
            [torch.max(h_heavy, dim=-1)[0], torch.max(h_light, dim=-1)[0]], dim=-1
        )
    ...
    def forward_alternative(self, x):
        ...
        h = self.ffd(
            torch.cat([torch.max(h_ab, dim=2)[0], torch.max(h_ag, dim=2)[0]], dim=1)
        )
...
class ESMModel(torch.nn.Module):
    ...
    def __init__(self, pretrained_esm2_model="esm2_t6_8M_UR50D"):
        ...
        self.esm, self.alphabet = getattr(esm.pretrained, self.pretrained_esm2_model)()
        self.batch_converter = self.alphabet.get_batch_converter()
        self.final_layer = int(self.pretrained_esm2_model.split("_")[1][1:])
    ...
    def forward(self, x):
        ...
        x = self.esm(x, repr_layers=[self.final_layer])["representations"][
            self.final_layer
        ]  # [B, L, n_outputs]
